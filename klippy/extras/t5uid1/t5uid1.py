...
class T5UID1:
    ...
    def is_busy(self):
        ...
        if self.probe is None:
            return False
        # Upstream removed multi_probe_pending; an open hardware probe session
        # is the equivalent "probing in progress" state.
        probe_session = getattr(self.probe, 'probe_session', None)
        return getattr(probe_session, 'hw_probe_session', None) is not None
